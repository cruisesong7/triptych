...
import CedarSupport.DecimalInternals
...
open CedarSupport.DecimalInternals
...
theorem toInt_parts_eq (sgn natural : String) (hsgn : sgn = "-" ∨ sgn = "")
    (hi : _root_.IsDigits natural) :
    toInt?' (sgn ++ natural) =
      some (Triptych.signOf sgn * Triptych.natOf natural) := by
  have hnat' := readNat_eq natural hi
  ...
  · unfold toInt?'
    ...
    right
    exact ⟨natural, rfl, Triptych.readNat natural, hnat', by
...
theorem decimal_match_mem (cs r : List Char) (m : CaptureMap) :
  ...
  · intro hmem
    ...
    obtain ⟨i, rfl, hi, hdi⟩ := (natural_mem 3 _ _ _).mp hI
  ...
  · rintro ⟨sgn, natural, fraction, hsgn, rfl, hcs, hi, hf⟩
...
theorem decode_decimal_sound {s : String} {m : CaptureMap}
  ...
  obtain ⟨sgn, natural, fraction, hsgn, hm, hs, hi, hf⟩ :=
    (decimal_match_mem s.toList [] m).mp hxmem
  refine ⟨sgn, natural, fraction, hsgn, hm, ?_, hi, hf⟩
  rw [← String.toList_inj]
...
theorem cedar_wf_to_surface {s : String} (h : Cedar.Thm.Decimal.IsWfDecimal s) :
    Decimal.IsWf.Decimal s := by
  rw [CedarSupport.DecimalInternals.isWfStr_iff] at h
  ...
  · rename_i hno
    ...
    rcases String.toInt?_eq_some_iff.mp hleftValue with
      ⟨leftNat, hleftNat, _⟩ | ⟨natural, hleftEq, naturalNat, hnaturalNat, _⟩
    ...
    · have hnaturalNoUnderscore : natural.contains '_' = false := by
        have hnot : ¬ ('_' ∈ natural.toList) := by
          ...
          contradiction
        simpa [String.contains] using hnot
      have hnaturalNat' : toNat?' natural = some naturalNat := by
        unfold toNat?'
        rw [hnaturalNoUnderscore]
        exact hnaturalNat
      have hi : Triptych.IsDigits natural :=
        (digits_iff natural).mpr
          (toNatIsSome_iff_isDigits.mp (by simp [hnaturalNat']))
      exact ⟨"-", natural, fraction,
...
theorem surface_wf_to_cedar {s : String} (h : Decimal.IsWf.Decimal s) :
    Cedar.Thm.Decimal.IsWfDecimal s := by
  rw [CedarSupport.DecimalInternals.isWfStr_iff]
  obtain ⟨sgn, natural, fraction, ⟨⟨hs, hsgn⟩, hi⟩, hf⟩ := h
  have hi' : _root_.IsDigits natural := (digits_iff natural).mp hi
  have hfDigits : Triptych.IsDigits fraction := ⟨hf.1, hf.2.1⟩
  have hf' : _root_.IsDigits fraction := (digits_iff fraction).mp hfDigits
  refine ⟨sgn ++ natural, fraction, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · rw [hs]
    apply split_decimal
    · intro c hc
  ...
  · have hmin := hf.2.1
    omega
  · exact hf.2.2
  · rw [toInt_parts_eq sgn natural hsgn hi']
    simp
  · rw [readNat_eq fraction hf']
    simp

theorem format_compute_of_decode (s sgn natural fraction : String)
    (h : decode Decimal.grammar s =
      some [("Sign", sgn), ("Natural", natural), ("Fraction", fraction)]) :
    Decimal.computeValue s = some (Decimal.value sgn natural fraction) := by
...
theorem cedar_compute_of_parts (s sgn natural fraction : String)
    (hs : s = sgn ++ natural ++ "." ++ fraction)
    (hsgn : sgn = "-" ∨ sgn = "")
    (hi : Triptych.IsDigits natural)
    (hf : Triptych.IsDigitsBetween 1 4 fraction) :
    Cedar.Thm.Decimal.computeValue s = some (Decimal.value sgn natural fraction) := by
  have hi' : _root_.IsDigits natural := (digits_iff natural).mp hi
  have hfDigits : Triptych.IsDigits fraction := ⟨hf.1, hf.2.1⟩
  have hf' : _root_.IsDigits fraction := (digits_iff fraction).mp hfDigits
  have hleftNoDot :
  ...
  · simp only [String.empty_append, Triptych.signOf]
    have hnot : ¬ ['-'] <+: natural.toList := by
      ...
      have := hi'.2 '-' hm
      simp at this
    have hstarts : natural.startsWith "-" = false :=
...
theorem decode_parts_of_surface_wf {s : String} (h : Decimal.IsWf.Decimal s) :
  ...
  obtain ⟨sgn, natural, fraction, hsgn, rfl, hs, hi, hf⟩ :=
    decode_decimal_sound hm
  exact ⟨sgn, natural, fraction, hm, hsgn, hs, hi, hf⟩

theorem bridge_value (s : String) (h : Cedar.Thm.Decimal.IsWfDecimal s) :
    Decimal.computeValue s = Cedar.Thm.Decimal.computeValue s := by
  have hsurface := cedar_wf_to_surface h
  obtain ⟨sgn, natural, fraction, hdecode, hsgn, hs, hi, hf⟩ :=
    decode_parts_of_surface_wf hsurface
  rw [format_compute_of_decode s sgn natural fraction hdecode,
    cedar_compute_of_parts s sgn natural fraction hs hsgn hi hf]

theorem constraints_of_decode (s sgn natural fraction : String)
    (h : decode Decimal.grammar s =
      some [("Sign", sgn), ("Natural", natural), ("Fraction", fraction)]) :
...
theorem bridge_isValid (s : String) :
    Decimal.IsValid s ↔
      Cedar.Thm.Decimal.IsWfDecimal s ∧
        ∃ v, Cedar.Thm.Decimal.computeValue s = some v ∧
  ...
  · rintro ⟨hwf, hconstraints⟩
    ...
    have hvalue :
        Cedar.Thm.Decimal.computeValue s = some (Decimal.value sgn natural fraction) := by
      rw [← bridge_value s hcedar]
      exact hformat
    exact ⟨hcedar, Decimal.value sgn natural fraction, hvalue,
      (constraints_of_decode s sgn natural fraction hdecode).mp hconstraints⟩
  · rintro ⟨hcedar, v, hvalue, hrange⟩
    have hwf := cedar_wf_to_surface hcedar
    obtain ⟨sgn, natural, fraction, hdecode, _, _, _, _⟩ :=
      decode_parts_of_surface_wf hwf
    have hformat := format_compute_of_decode s sgn natural fraction hdecode
    have hv : Decimal.value sgn natural fraction = v := by
