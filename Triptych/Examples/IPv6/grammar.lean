...
`Triptych.decodeSome_iff_IsWf`): the separator must be non-empty and the lower bound ≥ 1.
Both hold here (`":"`, `{8}`); the DSL rejects violations at parse time. Writes `spec.lean` +
`parser.lean` beside this file (no `parser`/`printer` clause, so no `soundness.lean`).
-/
