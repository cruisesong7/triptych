...
Run: `lake env lean FormatSpec/Examples/ConformanceTests.lean` (or build the module — the `#eval`
...
-- surface `IsValid` Prop) rather than the doubly-qualified `FormatSpec.Examples.Decimal.Decimal.parse`.
...
namespace FormatSpec.ConformanceTests
...
def runAll : IO Nat := do
  IO.println "════════ FormatSpec ↔ Cedar parser conformance ════════"
...
end FormatSpec.ConformanceTests
...
#eval do
  let failures ← FormatSpec.ConformanceTests.runAll
  if failures != 0 then
