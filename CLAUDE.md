...
- **Axioms:** every generated equivalence/decidability result must depend ONLY on
  ...
  `soundness.lean` of examples with a `parser`/`printer` clause (currently 15): the 3
  ...
  the generated parser through `σ`; = Cedar's `Int64.ofInt_toInt` for Decimal). So: Decimal 6 +
  Duration 6 + Datetime 3 = 15. The printer theorems derived from those obligations
...
- `Examples/{Decimal,Duration,Datetime,IPv4,IPv6,Graph}/` — each `grammar.lean` runs
  ...
  `encode_*`/`lift_section` obligations + the discharged `parse_toString_*` printer theorems about
...
- Values: `value <arith>` (scalar Int DSL) OR `value' f X Y` escape returning ANY type
  ...
  never see a new constructor; detection of sign productions is SYNTACTIC (in `elabFormatSpec`).
- Clause nesting (two duals): `lift <σ>` is a SUB-clause of `value` (parses only after a scalar
  ...
  section/retraction pair (`σ ∘ π = id`, the `lift_section` obligation). So: `projection` is
